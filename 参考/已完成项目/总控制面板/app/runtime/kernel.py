...
All LLM/Tool/Prompt logic preserved as-is (temporary compat — will be
extracted to Scheduler/ContextEngine/Capability in future tasks).

DO NOT add: prompt building, memory storage, LLM routing, policy checks.
Those belong to ContextEngine/Memory/Scheduler/Governor.
"""
import re
import time
import uuid
...
# ── WorkingMemory (temporary compat — → ContextEngine in future task) ──
class WorkingMemory:
...
# ── System prompt (temporary compat — → ContextEngine in future task) ──
...
class MotherRuntime:
    ...
    def run(self, message: str, session_id: int = 0,
        ...
        Args:
            ...
            llm_client: Optional LLMClient for backward compat (agent_run path).
                        If provided, used for LLM calls instead of _build_candidates.
    ...
    def _execute(self, message: str, session_id: int,
        ...
        for turn in range(max_turns):
            # LLM call: use llm_client if provided (agent_run path),
            # else use token_pool candidates (Gateway path)
            raw = None
            last_err = ""

            if llm_client:
                raw, last_err = self._call_with_client(llm_client, wm)
            else:
                raw, last_err = self._call_with_pool(wm)
    ...
    def _call_with_client(self, llm_client, wm: WorkingMemory):
        """LLM via injected LLMClient (agent_run compat path)."""
        try:
            resp = httpx.post(
    ...
    def _call_with_pool(self, wm: WorkingMemory):
        """LLM via token_pool candidates (Gateway compat path)."""
        candidates = self._build_candidates()
        last_err = ""
        for base_url, api_key, model in candidates[:4]:
            try:
                r = httpx.post(
    ...
    def _build_candidates(self) -> list[tuple]:
        """Build LLM key candidates from token_pool (temporary compat)."""
        candidates = []
        seen = set()
        try:
    ...
    def _execute_tool(self, name: str, arg: str) -> str:
        """Execute tool — route to app.skills or app.tools (temporary compat)."""
        try:
